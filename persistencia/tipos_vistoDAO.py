...
class Tipos_VistoDAO:
    def __init__(self):        
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS visa_documents (
                visa_name TEXT,
                nomedoc TEXT,
                FOREIGN KEY (visa_name) REFERENCES tipos_visto(id),
                FOREIGN KEY (nomedoc) REFERENCES teste(nomedoc)
            )
        ''')
        self.conn.commit()
    ...
    def cadastrar_documentos_para_visto(self, docs, visa_name):
        for doc in docs:
            self.cursor.execute("INSERT INTO visa_documents (visa_name, nomedoc) VALUES (?, ?)", (visa_name, doc['nomedoc']))
            self.conn.commit()
